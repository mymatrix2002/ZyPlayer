...
export const ZY_NODE_PROXY_RULES_ENV = 'ZY_NODE_PROXY_RULES';
export const ZY_NODE_PROXY_BYPASS_RULES_ENV = 'ZY_NODE_PROXY_BYPASS_RULES';

const NODE_PROXY_ENV_KEYS = [
  ZY_NODE_PROXY_RULES_ENV,
  ZY_NODE_PROXY_BYPASS_RULES_ENV,
  'HTTP_PROXY',
  'HTTPS_PROXY',
  'http_proxy',
  'https_proxy',
...
export const getNodeProxyConfigFromEnvironment = (env: NodeJS.ProcessEnv = process.env): NodeProxyConfig | null => {
  ...
  const proxyRules =
    proxyEnv[ZY_NODE_PROXY_RULES_ENV] ||
    proxyEnv.ALL_PROXY ||
    proxyEnv.all_proxy ||
  ...
  return {
    proxyRules,
    proxyBypassRules: proxyEnv[ZY_NODE_PROXY_BYPASS_RULES_ENV] || proxyEnv.NO_PROXY || proxyEnv.no_proxy,
  };
};
...
export const buildNodeProxyEnvironment = (config: NodeProxyConfig): Record<string, string> => {
  ...
  const env: Record<string, string> = {
    [ZY_NODE_PROXY_RULES_ENV]: proxyUrl,
    [ZY_NODE_PROXY_BYPASS_RULES_ENV]: normalizedByPassRules.join(','),
  };
...
export class NodeProxyController {
  ...
  private setEnvironment(url: string | undefined, normalizedByPassRules: string[]): void {
    delete process.env[ZY_NODE_PROXY_RULES_ENV];
    delete process.env[ZY_NODE_PROXY_BYPASS_RULES_ENV];
